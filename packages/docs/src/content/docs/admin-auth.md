...
3. O BFF define os cookies `naulite_session` e `naulite_csrf` e devolve `csrfToken` no corpo da resposta.
...
|----------|---------|-----------|
| `NAULITE_SESSION_TTL_SECONDS` | `86400` | Tempo de vida da sessão admin |
| `NAULITE_COOKIE_SECURE` | `true` em `NODE_ENV=production` | Adiciona `Secure` nos cookies do BFF |
| `ADMIN_API_KEY` | - | Token legado para automação via `Authorization: Bearer` |

## Feature flags (opt-in, default desligado)

### `NAULITE_MULTI_TENANT`
...
### `NAULITE_API_KEY_ROTATION_ENABLED`

Quando `true`, habilita `POST /api-keys/:id/rotate` com período de graça (`NAULITE_API_KEY_ROTATION_GRACE_SECONDS`). Com `false` (default), a rota retorna `503`.
