...
export class ControlPlaneSync {
    ...
    async publish(eventType: string, payload: Record<string, unknown>): Promise<ControlPlaneSyncEvent | null> {
        ...
        const rows = await this.databaseProvider.getPgDb().insert(postgresSchema.controlPlaneEvents).values({
            eventType,
            payload,
    ...
    async poll(): Promise<void> {
        ...
        const rows = await this.databaseProvider.getPgDb()
            .select()
            .from(postgresSchema.controlPlaneEvents)
            .where(gt(postgresSchema.controlPlaneEvents.id, this.lastEventId))
            .orderBy(desc(postgresSchema.controlPlaneEvents.id));
