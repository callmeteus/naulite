...
pub fn applyManifest(allocator: std.mem.Allocator, client: *Client, manifest_path: []const u8) !void {
    const stdout = std.io.getStdOut().writer();
    const manifest = try std.fs.cwd().readFileAlloc(allocator, manifest_path, 8 * 1024 * 1024);
    defer allocator.free(manifest);

    var body = std.ArrayList(u8).init(allocator);
    defer body.deinit();

    try std.json.stringify(.{ .manifest = manifest }, .{}, body.writer());

    const response = try client.postJson("/apply", body.items);
    defer allocator.free(response.body);
...
pub fn execInstance(allocator: std.mem.Allocator, client: *Client, instance_id: []const u8, command_argv: []const []const u8) !u8 {
    ...
    var body = std.ArrayList(u8).init(allocator);
    defer body.deinit();
    try std.json.stringify(.{ .command = command_argv }, .{}, body.writer());

    const response = try client.postJson(path, body.items);
    defer allocator.free(response.body);
...
pub fn triggerBuild(
    ...
    const response = try client.postJson("/build", body.items);
    defer allocator.free(response.body);
    try printJson(stdout, response.body);
}
